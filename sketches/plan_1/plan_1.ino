...
void loop(){
  ...
  float ActualRPM = velocityComputaion();
  ...
  float ErrorRPM = abs(IdealRPM - ActualRPM);
...
//  Serial.println ("IdealRPM","+String(IdealRPM)+",ActualRPM,"+String(ActualRPM)+",ErrorRPM,"String(ErrorRPM));// does not like the string part 
...
float velocityComputaion(){
  ...
  return(float)(vel * 60 / ENC_COUNT_REV);
